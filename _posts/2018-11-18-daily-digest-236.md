---
layout: post
title: "Daily Digest &#35;234"
type: digest
tags: 
    - daily
    - development
    
description: ""
publish: true
---

- [Three Sales Mistakes Software Engineers Make](https://www.pipelinedb.com/blog/three-sales-mistakes-software-engineers-make)
- [How Drift Gets Engineers up to Speed by Day 3](https://blog.gitprime.com/drift-3-day-onboarding-process/)
- [Launching Your Own Ruby Gem - Part 1: Build It](https://buttercms.com/blog/launching-your-own-ruby-gem-build-it)
- [Why React’s new Hooks API is a game changer](https://itnext.io/why-reacts-hooks-api-is-a-game-changer-8731c2b0a8c)
- [4 Anonymous Web Browsers That Are Completely Private](https://www.makeuseof.com/tag/2-anonymous-web-browsers-completely-private-secure/)
- [Dive -- A tool for exploring each layer in a docker image](https://github.com/wagoodman/dive)
- [A Tour of The Top 10 Algorithms for Machine Learning Newbies](https://towardsdatascience.com/a-tour-of-the-top-10-algorithms-for-machine-learning-newbies-dde4edffae11)
- [CloudWatch Is of the Devil, but I Must Use It](https://www.linuxjournal.com/content/cloudwatch-devil-i-must-use-it)
