---
layout: post
title: "Daily Digest &#35;45"
type: digest
tags: 
    - daily
    - development
description: ""
publish: true
---

- [Ruby is alive and well and thinking about the next 25 years](https://www.techradar.com/news/ruby-is-alive-and-well-and-thinking-about-the-next-25-years)
- [Python Logging: An In-Depth Tutorial](https://www.toptal.com/python/in-depth-python-logging)
- [Serverless, and the challenges using it ](https://capgemini.github.io/.net/serverless-and-using-it/)
- [LinkedIn Lite: A Lightweight Mobile Web Experience](https://engineering.linkedin.com/blog/2018/03/linkedin-lite--a-lightweight-mobile-web-experience)
- [Using Machine Learning to Improve Streaming Quality at Netflix](https://medium.com/netflix-techblog/using-machine-learning-to-improve-streaming-quality-at-netflix-9651263ef09f)
- [A Beginner’s Guide to IPFS](https://hackernoon.com/a-beginners-guide-to-ipfs-20673fedd3f)
- [Introducing QALM, Uber’s QoS Load Management Framework](https://eng.uber.com/qalm/#utm_source=jakartadev)
